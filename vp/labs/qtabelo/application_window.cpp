...
bool ApplicationWindow::exportTableToCvs(QTableWidget *table, const QString &filename) {
    QFile file(filename);
...
bool ApplicationWindow::importCvsToTable(const QString &filename, QTableWidget *table) {
    QFile file(filename);
...
bool ApplicationWindow::loadDocument(const QUrl &url)
{
    ...
    if (document->findChild<QTabWidget*>()) {
    ...
    if (!importCvsToTable(url.toLocalFile(), table)) {
        document->close();
        return false;
    }
...
bool ApplicationWindow::saveDocument(DocumentWidget *document, const QUrl &altUrl)
{
   // Q_UNUSED(resetModified());
    ...
    if (!exportTableToCvs(table, url.toLocalFile())) {
