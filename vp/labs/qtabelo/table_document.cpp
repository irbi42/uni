...
void TableDocument::showTableContextMenu(QTableWidget *table, const QPoint &pos) {
    ...
    QAction *actionRemoveRow = menu.addAction(tr("Remove Row"));
    QAction *actionAddColumn = menu.addAction(tr("Add Column"));
    QAction *actionRemoveColumn = menu.addAction(tr("Remove Column"));
    QAction *actionCreateBarChart = menu.addAction(tr("Create Bar Chart"));
    ...
    if(selectedAction == actionAddRow) {
        ...
        emit contentChanged(true);
    } else if (selectedAction == actionCreateBarChart && !table->selectedItems().isEmpty()) {
        ...
        int tabIndex = m_tabs->addTab(chartView, tr("Diagramm"));
...
void TableDocument::slotAddTab(const int count)
{
    if (m_tabs->count() == 0) {
        ...
        for (int i = 1; i <= count; ++i) {
            ...
            m_tabs->addTab(table, tr("Sheet %1").arg(i));
        }
